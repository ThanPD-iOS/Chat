...
//            }
            ...
            VStack(alignment: message.messageRole == .user ? .trailing : .leading, spacing: 2) {
                if !isDisplayingMessageMenu, let reply = message.replyMessage?.toMessage() {
                    replyBubbleView(reply)
                        ...
                        .padding(message.messageRole == .user ? .trailing : .leading, 10)
                        .overlay(alignment: message.messageRole == .user ? .trailing : .leading) {
                            Capsule()
        ...
        .padding(.trailing, message.messageRole == .user ? MessageView.horizontalNoAvatarPadding : 0)
        .padding(
            message.messageRole == .user ? .leading : .trailing, MessageView.horizontalBubblePadding
        )
        .frame(
            maxWidth: UIScreen.main.bounds.width,
            alignment: message.messageRole == .user ? .trailing : .leading)
    }
...
//    }
    ...
    func recordingView(_ recording: Recording) -> some View {
        RecordWaveformWithButtons(
            recording: recording,
            colorButton: message.messageRole == .user
                ? theme.colors.messageMyBG : theme.colors.mainBG,
            colorButtonBg: message.messageRole == .user
                ? theme.colors.mainBG : theme.colors.messageMyBG,
            colorWaveform: theme.colors.messageText(message.messageRole)
        )
        .padding(.horizontal, MessageView.horizontalTextPadding)
        .padding(.top, 8)
    }
